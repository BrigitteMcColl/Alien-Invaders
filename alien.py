...
class Alien(Sprite):   # INHERITS from SPRITE
    images = [[pg.image.load('images/invader' + str(number) + str(i) + '.png') for i in range(2)] for number in range(3)]
    images_boom = [pg.image.load('images/Alien_boom' + str(i) + '.png') for i in range(4)]
    images_ufo = [pg.image.load('images/ufo' + str(i) + '.png') for i in range(5)]
    timers = []
    for i in range(3):
        timers.append(Timer(frames=images[i], wait=700))
    timer_boom = Timer(frames=images_boom, wait=100, looponce=True)
    for i in range(1):
        timers.append(Timer(frames=images_ufo, wait=(i + 1) * 700))
